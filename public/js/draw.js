...
// clear all function
   // ✅ Clear All Tool
const clearTool = document.querySelector("img[src='NewIcons/clear.png']");

if (clearTool) {
  clearTool.addEventListener("click", function () {
    // Pehle confirm alert dikhana
    let confirmClear = confirm("⚠️ Are you sure you want to clear the canvas? This cannot be undone.");
    
    if (confirmClear) {
      // Canvas ko clear karo
      ctx.clearRect(0, 0, board.width, board.height);

      // Undo/Redo stack ko reset karo
      undoStack = [];
      redoStack = [];

      // Real-time clear event bhejo (agar multi-user hai)
      socket.emit("clearAll");
    }
  });
}

// ✅ Socket event listener (dusre users ke liye bhi clear ho jaye)
